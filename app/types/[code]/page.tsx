...
function axisBar(label: string, value: number, left: string, right: string) {
  return (
    <div className="space-y-2 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
      <div className="flex items-center justify-between text-xs text-slate-500"><span>{left}</span><span>{right}</span></div>
      <div className="text-sm font-medium text-slate-900">{label}</div>
      <div className="h-3 w-full overflow-hidden rounded-full bg-slate-200"><div className="h-full rounded-full bg-slate-900" style={{ width: `${value}%` }} /></div>
      <div className="text-right text-xs text-slate-500">{value}%</div>
    </div>
  );
}
    ...
    <main className="mx-auto max-w-5xl space-y-8 px-4 pb-12 pt-[calc(env(safe-area-inset-top)+20px)] md:px-6 md:pb-16 md:pt-8">
      <section className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          <Button asChild variant="outline" className="rounded-full"><Link href="/types">← 回到所有類型</Link></Button>
          <Badge variant="secondary" className="rounded-full px-3 py-1 text-sm">{profile.family}</Badge>
          <Badge className="rounded-full px-3 py-1 text-sm">{profile.code}</Badge>
        </div>
        <Card className="rounded-3xl border-slate-900 bg-slate-900 text-white shadow-lg">
          <CardHeader className="space-y-4">
            <CardDescription className="text-slate-300">拉麵口味風格</CardDescription>
            <div className="space-y-2">
              <CardTitle className="text-4xl tracking-tight md:text-5xl">{profile.code}</CardTitle>
              <div className="text-2xl font-semibold text-white md:text-3xl">{profile.name}</div>
            </div>
            <CardDescription className="max-w-3xl text-base leading-7 text-slate-200">{profile.summary}</CardDescription>
          </CardHeader>
        </Card>
      </section>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <Card className="rounded-2xl border-slate-200 bg-white shadow-sm">
          ...
          <CardContent className="space-y-4">
            ...
            <div className="rounded-2xl bg-slate-50 p-4 text-sm leading-7 text-slate-700">{profile.short}</div>
          </CardContent>
        </Card>
        <Card className="rounded-2xl border-slate-200 bg-white shadow-sm">
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div className="flex flex-wrap gap-2">
              {profile.ingredientTags.map((tag) => <Badge key={tag} variant="secondary" className="rounded-full px-3 py-1 text-sm">{tag}</Badge>)}
            </div>
            <div className="text-sm leading-7 text-slate-600">如果你測出這型，通常代表你不只是喜歡某一種肉，而是會反覆被這些食材方向吸引。</div>
          </CardContent>
        </Card>
      </section>
          ...
          <CardContent className="space-y-3">
            {relatedPrototypes.map((item) => (
              <div key={item.id} className="rounded-2xl bg-slate-50 p-4">
                ...
                <div className="mt-1 text-sm leading-6 text-slate-600">{item.short}</div>
              </div>
            ))}
          </CardContent>
        </Card>
              ...
              <Link key={item.code} href={`/types/${item.code}`} className="block">
                <div className="rounded-2xl border border-slate-200 p-4 transition hover:bg-slate-50">
                  <div className="flex items-center justify-between gap-3">
                    <div><div className="text-sm text-slate-500">{item.code}</div><div className="font-medium text-slate-900">{item.name}</div></div>
                    <Badge variant="secondary" className="rounded-full px-3 py-1 text-xs">{item.family}</Badge>
                  </div>
                  <div className="mt-2 text-sm leading-6 text-slate-600">{item.short}</div>
                </div>
              </Link>
            ))}
          </CardContent>
      ...
      <section>
        <Card className="rounded-2xl border-slate-200 bg-slate-50/60 shadow-sm">
          ...
          <CardContent className="grid gap-3 md:grid-cols-3">
            <HomeButton />
            <Button asChild variant="outline" className="rounded-xl"><Link href="/types">回所有類型</Link></Button>
            <Button asChild variant="outline" className="rounded-xl"><Link href={`/types/${profile.neighborCodes[0]}`}>看看接近型</Link></Button>
          </CardContent>
        </Card>
      </section>
    </main>
